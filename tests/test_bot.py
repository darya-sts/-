...
def test_start_menu_has_action_buttons() -> None:
    ...
    assert "MCP_SERVER_URL / MCP_SERVER_COMMAND / MCP_API_KEY" not in menu.COMMANDS_HELP


def test_telegram_chat_has_no_deepseek_dialog() -> None:
    text = (ROOT / "telegram_agent" / "src" / "bot" / "handlers" / "telegram.py").read_text(
        encoding="utf-8"
    )
    assert "instance.agent.chat" not in text
    assert "await instance.bot.send(msg, COMMANDS_HELP)" in text
    assert "parse_writer_message" in text
