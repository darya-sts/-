...
Одна строка — один человек: числовой Telegram ID или `@username`. Пустые строки и комментарии с `#` игнорируются. Если список пуст, бот никому не отвечает.
...
Дополнительно можно задать `ALLOWED_USERS` в `.env` или в панели Timeweb, через запятую: `123456789,@my_username`.
