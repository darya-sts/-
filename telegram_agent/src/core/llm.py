...
LLM_ORDER = _order("LLM_ORDER", "deepseek,opencode-alt,opencode,gemini,gemini-small")
LLM_ORDER_FAST = _order("LLM_ORDER_FAST", "deepseek,opencode-alt,opencode,gemini-small")
...
class LLM(Singleton):
    ...
    def get(provider: str | None = None) -> BaseChatModel:
        ...
        if not obj.llm:
            ...
            api_key_deepseek: Any = getenv("DEEPSEEK_API_KEY")
            model_deepseek = SPECS["deepseek"][0]
            if api_key_deepseek and model_deepseek:
                deepseek_kwargs: dict[str, Any] = {
                    "api_key": api_key_deepseek,
                    "model": model_deepseek,
                    "disable_streaming": "tool_calling",
                }
                deepseek_base = getenv("DEEPSEEK_API_BASE", "").strip()
                if deepseek_base:
                    deepseek_kwargs["api_base"] = deepseek_base
                obj.llm["deepseek"] = ChatDeepSeek(**deepseek_kwargs)
